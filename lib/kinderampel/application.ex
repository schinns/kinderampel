defmodule Kinderampel.Application do
  ...
  def start(_type, _args) do
    ...
    opts = [strategy: :one_for_one, name: Kinderampel.Supervisor]

    children =
      [
        # Children for all targets
        # Starts a worker by calling: Kinderampel.Worker.start_link(arg)
        # {Kinderampel.Worker, arg},
      ] ++ children(target())
  ...
  def start_ssh() do
    # Reuse keys from `nerves_ssh` so that the user only needs one
    # config.exs entry.
    authorized_keys =
